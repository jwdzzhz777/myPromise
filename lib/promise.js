...
var StatesEnum;
(function (StatesEnum) {
    StatesEnum["pending"] = "pending";
    StatesEnum["fulfilled"] = "fulfilled";
    StatesEnum["rejected"] = "rejected";
...
class MyPromise {
        ...
        let promise2 = new MyPromise((resolve, reject) => {
            ...
            if (this.state === StatesEnum.rejected) {
                ...
                if (!(onRejected instanceof Function))
                    reject(this.reason);
                generator(onRejected, this.reason)();
            }
        });
        return promise2;
    }
     ...
     * 处理 promise状态
     */
    static [Reslove](promise, x) {
        let called = false; // 防止多次调用
        /** 如果 promise 和 x 指向同一对象，以 TypeError 为据因拒绝执行 promise */
        if (promise === x)
            return promise[reject](new TypeError('不能返回自己'));
    ...
    static resolve() { }
    static reject(err) { }
    get [(_a = resolve, _b = reject, Symbol.toStringTag)]() {
        return 'Promise';
    }
}
